...
describe Thron::Gateway::VUserManager do
  ...
  it 'must call get to fetch user detail' do
    ...
    extra = entity::new(return_itags: false, return_imetadata: true).to_payload
    query = { 
      clientId: instance.client_id,
      username: username,
      offset: 7,
    ...
    instance.user_detail(username: username, extra: extra, offset: 7, limit: 5)
  end
  ...
  it 'must call post to find users by properties' do
    ...
    fields_option = entity::new(return_own_acl: false, return_itags: true, return_imetadata: true).to_payload
    body = { 
      clientId: instance.client_id,
      criteria: criteria,
      orderBy: 'name',
    ...
    mock(klass).post(route.url, { query: {}, body: body, headers: route.headers(token_id: token_id, dash: true) }) { response }
    instance.find_users(criteria: criteria, order_by: 'name', fields_option: fields_option, limit: 30)
  end
