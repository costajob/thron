...
describe Thron::Gateway::Contact do
  ...
  it 'must call post to list existing contacts' do
    ...
    option = entity::new(itags: false, keys: true, old_ids: false).to_payload
    body = {
      criteria: criteria,
      option: option,
      offset: 40,
    ...
    mock(klass).post(route.url, { query: {}, body: body, headers: route.headers(token_id: token_id, dash: true) }) { response }
    instance.list_contacts(criteria: criteria, option: option, offset: 40, limit: 20)
  end
