...
describe Thron::Gateway::UsersGroupManager do
  ...
  it 'must call post to get group detail' do
    ...
    mock(klass).post(route.url, { query: {}, body: body, headers: route.headers(token_id: token_id, dash: true) }) { response }
    instance.group_detail(group_id: group_id, fields_option: fields_option, offset: 3, limit: 5)
  end
  ...
  it 'must call post to find group by properties' do
    ...
    mock(klass).post(route.url, { query: {}, body: body, headers: route.headers(token_id: token_id, dash: true) }) { response }
    instance.find_groups(criteria: criteria, order_by: 'id', fields_option: fields_option)
  end
