    ...
    class User
      def self.mappings
        @mappings ||= { 
          ...
          credentials: Attribute::new(name: 'credential', type: :Plain),
          patches: Attribute::new(name: 'patch', type: [:Plain]),
          linked_groups_count: Attribute::new(name: 'totalLinkedGroups', type: Attribute::INT),
          linked_groups: Attribute::new(name: 'linkedGroups', type: [:Group])
        }
      end
      include Mappable
    end
